setwd("/path/to/data") # Change to your project folder path
...
## +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## The GAM we used in the end can be seen below. We adapted the GAM according to
## Jordan Dworkin's notes above. Further, we added journal as interaction term
## for the age of the paper, since the journals were first published at
## considerably different dates. Due to this interaction the computation 
## of the model is time consuming.

journal = as.factor(journal)
genderGAM=gam(list(gender_cat~s(month_from_base, k=10,by=journal)+s(log_seniority, k=10)+
                    s(log_teamsize, k=15)+journal+review,
                  ~s(month_from_base, k=10,by=journal)+s(log_seniority, k=10)+
                    s(log_teamsize, k=15)+journal+review,
                  ~s(month_from_base, k=10,by=journal)+s(log_seniority, k=20)+
                    s(log_teamsize, k=10)+journal+review),family=multinom(K=3))
