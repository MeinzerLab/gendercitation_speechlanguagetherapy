...
if("df6_namegends.RData"%in%list.files()){
  ...
  # Fill in some data using pre-built csumommon names database
...
for(i in r){
  ...
  ## +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ## The alternative API required us to change the order of this if loop and to
  ## implement the "is.null"-condition instead of an else. The API returned the
  ## "probability" of a name instead of "accuracy". Therefore, we also changed this
  ## within the code. Note that "accuracy" and "probability" mean the same in
  ## this instance.
  ## Original code:
  ...
  if(is.null(json_data$gender)){
    namegends$prob.w[i]=-1
    namegends$prob.m[i]=-1
  }else if(json_data$gender=="male"){
    namegends$prob.m[i]=json_data$probability/100
    namegends$prob.w[i]=1-json_data$probability/100
  }else if(json_data$gender=="female"){
    namegends$prob.w[i]=json_data$probability/100
    namegends$prob.m[i]=1-json_data$probability/100
  }
  ...
  print(paste0(this_name,"=",json_data$gender,
               ": confidence=",json_data$probability))
